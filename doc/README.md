...
> 真正开发顺序以项目 `plan/阶段路线图.md` 的 TODO 为准；继续开发前必须先读 `plan/当前实现审计.md`，确认代码当前到底是 Mock、占位还是真实实现。
...
| 文档 | 作用 |
|---|---|
| `00-文档入口.md` | 唯一入口。新窗口先读它，能知道产品做成什么、从哪里开始、哪些文档不要优先读 |
| `开始之前/这个产品是什么.md` | 理解产品定位、当前主线、成功标准 |
| `开始之前/最终产品形态.md` | 定义最终要做成什么样、菜单形态和参考项目最终取舍 |
| `开始之前/菜单与页面详细说明.md` | 定义每个菜单和页面的区域、按钮、状态、逻辑 |
| `UI设计/全页面布局设计.md` | 前端页面布局、导航和交互的当前执行口径；与旧 UI 草案冲突时以此为准 |
| `开始之前/主流程与数据流.md` | 定义从输入到成片的数据流和关键写回字段 |
| `开始之前/功能地图.md` | 看所有功能模块、优先级、模块边界、阅读顺序 |
| `功能模块/README.md` | 按当前优先级整理全部功能模块，开发前先看 |
| `../plan/阶段路线图.md` | 当前真实开发 TODO 和执行顺序 |
| `../plan/当前实现审计.md` | 当前代码真实状态：已实现、Mock、占位、待纠偏 |
...
| `底层设计/22-UI详细设计与页面布局.md` | UI token、响应式和底层施工细节；页面/交互执行口径以 `UI设计/全页面布局设计.md` 为准 |
...
| 文档 | 作用 |
|---|---|
| `参考分析/参考项目取舍总则.md` | 规定每个参考项目学什么、不学什么，防止做成四不像 |
| `参考分析/参考方法与防漏机制.md` | 规定如何参考项目才能不漏按钮、状态、异常、数据和后续功能 |
| `参考分析/参考项目深度机制与吸收口径.md` | 记录 waoowaoo、Toonflow-web 等参考项目具体怎么实现流程、分镜、提示词、候选、任务态、资产和设置分层，以及本项目怎么筛选吸收 |
| `参考分析/功能借鉴矩阵.md` | 核心功能账本：每个模块参考来源、按钮、状态、逻辑、TODO 映射 |
| `参考分析/页面交互账本.md` | 页面级交互账本：每个页面的区域、按钮、状态、抽屉/弹窗和参考来源 |
| `参考分析/MVP简化口径.md` | 当前 MVP 必做与后续增强边界，避免版本/hash/snapshot 等后期能力拖慢第一版 |
| `参考分析/版权与二开边界.md` | 明确只借鉴思路，不复制代码、UI、品牌、素材和 Skill 正文 |
| `参考分析/功能对比总表.md` | 历史功能对照稿，只作为证据补充，不决定最终蓝图 |
| `参考分析/两个项目怎么做的.md` | 源码证据索引，按需确认文件和函数，不作为实现指令 |
...
后续新增文档优先放入这四类目录，保持目录直白、按功能找文档；不要再新增新的入口型文档。

入口规则：

```text
doc/00-文档入口.md 是唯一入口。
新窗口、新对话、新开发者先读它。
旧架构 HTML、旧 UI 原型、参考项目源码证据都不是入口。
```
...
旧架构图和旧 UI 原型已删除。功能对比和源码证据只能作为补充材料，不能覆盖 `00-文档入口.md`、`开始之前/最终产品形态.md`、`开始之前/菜单与页面详细说明.md`、`开始之前/主流程与数据流.md`、`参考分析/参考项目深度机制与吸收口径.md`、`功能模块/README.md`、`plan/当前实现审计.md` 和当前 TODO。
...
3. 每个“怎么做”都要有出处：先看本项目蓝图、功能账本和页面账本，再按需查源码证据。
4. 不凭空编文件名、函数名、字段名。
